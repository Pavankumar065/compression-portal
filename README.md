...
**Demo Video:**
Watch a live demonstration of the Compression Portal's features and functionality on YouTube:
▶️ [**https://youtu.be/x9nNb302w1Q**](https://youtu.be/x9nNb302w1Q)
...
```bash
git clone [https://github.com/Pavankumar065/compression-portal.git](https://github.com/Pavankumar065/compression-portal.git)
...
2. **Clean (Optional):** `rmdir /s /q node_modules` & `del package-lock.json`
...
2. **Clean (Optional):** `rmdir /s /q node_modules`, `del package-lock.json`, `rmdir /s /q build`

3. **Create .env:** In frontend/, create .env with REACT_APP_API_BASE_URL=
...
2. **Deployed Demo:** Access the live, deployed version of the application at: [**https://my-compression-portal-app-pavan.onrender.com/**](https://my-compression-portal-app-pavan.onrender.com/).
...
3. **Frontend API URL:** Verify frontend/src/components/CompressionControls.js uses process.env.REACT_APP_API_BASE_URL || 'http://localhost:5000'.

4. **Local .env:** Ensure frontend/.env is in frontend/.gitignore and is left empty locally for deployment.

**Deployment Steps:**

1. **Sign Up / Log In to Render:** Go to <https://render.com/> and sign up or log in using your GitHub account.
